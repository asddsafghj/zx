...
> <b>Phone Number</b> : ]] .. phonenumbere, 1, "html")
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and (text:match("^[Pp]ing$") or text:match("^ربات$")) then
          if database:get("lang:gp:" .. msg.chat_id_) then
...
If you *want* to get the bot phone number , send command *Botphone* .]], 1, "md")
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and (text:match("^[Mm]enu$") or text:match("^مدیریت$")) and check_user_channel(msg) then
          local BotApi = tonumber(database:get("Bot:Api_ID"))
          if database:get("lang:gp:" .. msg.chat_id_) then
            ln = "En"
          else
        ...
        if is_leader(msg) and (text:match("^[Uu]pdate$") or text:match("^بروزرسانی$")) then
          if not database:get("lang:gp:" .. msg.chat_id_) then
            send(msg.chat_id_, msg.id_, 1, "*•نسخه مورد استفاده شما تهیه شده توسط Speed Team می باشد.\nبرای اطلاع از اپدیت به این کانال مراجعه فرمایید*: @SpeedChanel", 1,"md")
          else
            send(msg.chat_id_, msg.id_, 1, "_• You are using the version provided by 'Speed Team'_\n_Check out this Channel for updates : @SpeedChanel", 1, "md")
          end
        end
        if is_leader(msg) and (text:match("^[Vv]ersion$") or text:match("^نسخه فعلی$")) then
          local LastVer = "zeus bot"
          if LastVer then
            if not database:get("lang:gp:" .. msg.chat_id_) then
